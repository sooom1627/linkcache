...
**実装進捗（要約）**: ユーザーストーリー **US-A**（週次グラフの `daily_totals`・T1〜T9）は **完了**（[dashboard-overview-us-a.md](./dashboard-overview-us-a.md)）。**US-B の B1**（RPC の `daily_by_collection` 実装・[マイグレーション](../../supabase/migrations/20260322074231_get_dashboard_overview_daily_by_collection.sql)・MCP 適用）は **完了**（[dashboard-overview-us-b.md](./dashboard-overview-us-b.md)）。**US-B の B2〜B7**（型・Zod・フックでコレクション内訳を RPC に接続）および **US-C**（ドメイン内訳）、レジリエンス・引っ張り更新（**US-X**）は未。
...
- **データ** — [`useDashboardOverviewData.ts`](../../src/features/links/hooks/useDashboardOverviewData.ts): **チャートの週次系列**（`addedByDay` / `readByDay`）は **US-A T6 済** — [`useDashboardOverviewQuery`](../../src/features/links/hooks/useDashboardOverviewQuery.ts) 経由で RPC の `daily_totals` を使用。未取得時は 7 日ゼロプレースホルダ。RPC は **US-B B1 済** で `daily_by_collection` も返すが、**画面のコレクション内訳行列・`collectionStats` は未接続**（[`fetchDashboardOverview.api.ts`](../../src/features/links/api/fetchDashboardOverview.api.ts) は `daily_by_collection` を `z.array(z.unknown())` のみ — **B3/B4** で厳格化・合成）。**コレクション一覧・ドメイン集計**は引き続き `useCollections` と `useLinks({ limit: 500 })`。**内訳の日別行列**は [`mockAddedByDay` / `mockReadByDay`](../../src/features/links/utils/dashboardStats.ts) ＋ `splitDayTotalAcrossBuckets`（**US-B B4〜 / US-C** でサーバ系列へ）。コレクション行の `readCount` 仮計算（`Math.floor(n * 0.45)`）も同様に未置換。
...
**解釈**: **チャートの週次 added/read 系列**は T6 で RPC に接続済み。**DB 上**は **US-B B1** で `daily_by_collection` が埋まる。一方、**アプリのコレクション／ドメインの日別内訳**とサマリの仮 `readCount` は、コレクション側は **B4 まで**モック／クライアント仮計算のまま（ドメインは **US-C**）。`useLinks` の 500 件上限によるドメイン集計は**母集団を代表しない**可能性が引き続きある。

- **API（ダッシュ RPC・US-A T4 済）** — [`fetchDashboardOverview.api.ts`](../../src/features/links/api/fetchDashboardOverview.api.ts): `get_dashboard_overview` を `p_tz` 付きで呼び、Zod（`dashboardOverviewRpcSchema`）で `daily_totals`（7 件）ほかを検証。`daily_by_collection` は現状 **形の厳格検証なし**（`z.array(z.unknown())`）— **US-B B3** で [US-B §3](./dashboard-overview-us-b.md) の契約どおりにする。開発ビルドでは成功時に `__DEV__` のみ `console.log` でパース後 JSON を出力可能。
...
| データ合成（チャート系列・T6）          | **実装済み** — [`useDashboardOverviewData`](../../src/features/links/hooks/useDashboardOverviewData.ts) が `daily_totals` をチャート系列にマップし **`dashboardOverviewPending` / `dashboardOverviewFetching`** を返す、テスト [`useDashboardOverviewData.test.ts`](../../src/features/links/__tests__/hooks/useDashboardOverviewData.test.ts)。内訳日別はモック継続（US-B/C）                                                                                                                                                                                               |
...
| 検証記録                                | [dashboard-overview-us-a.md](./dashboard-overview-us-a.md) §5（T1〜T9 サマリ）、§6（T6〜T9）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- **削除対象（未・US-B/C）**: コレクション／ドメインの日別行列についての `mockAddedByDay` / `mockReadByDay` 依存、`splitDayTotalAcrossBuckets` による仮日別行列（サーバから実行列が来るなら）。
...
- **データ合成フック（T6・T8 追記）**: [`useDashboardOverviewData.test.ts`](../../src/features/links/__tests__/hooks/useDashboardOverviewData.test.ts) — `useDashboardOverviewQuery` 等を境界モックし、`daily_totals` とチャート系列の一致・内訳モック維持・未取得時ゼロ・**`dashboardOverviewPending` / `dashboardOverviewFetching`** を検証。fixtures の共通デフォルトは [`dashboardOverview.fixtures.ts`](../../src/features/links/testing/dashboardOverview.fixtures.ts)（`dashboardOverviewPending` / `dashboardOverviewFetching` のデフォルト含む）。
