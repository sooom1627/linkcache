...
**実装進捗（要約）**: ユーザーストーリー **US-A**（週次グラフの `daily_totals`・T1〜T9）は **完了**（[dashboard-overview-us-a.md](./dashboard-overview-us-a.md)）。内訳実データは **US-B / US-C**、レジリエンス・引っ張り更新は **US-X** が未。
...
**解釈**: **チャートの週次 added/read 系列**は T6 で RPC に接続済み。一方、**コレクション／ドメインの日別内訳**とサマリの仮 `readCount` はモック／クライアント仮計算のまま（US-B/C）。`useLinks` の 500 件上限によるドメイン集計は**母集団を代表しない**可能性が引き続きある。

- **API（ダッシュ RPC・US-A T4 済）** — [`fetchDashboardOverview.api.ts`](../../src/features/links/api/fetchDashboardOverview.api.ts): `get_dashboard_overview` を `p_tz` 付きで呼び、Zod（`dashboardOverviewRpcSchema`）で `daily_totals`（7 件）ほかを検証。開発ビルドでは成功時に `__DEV__` のみ `console.log` でパース後 JSON を出力可能。
