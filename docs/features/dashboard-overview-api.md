...
- **API（ダッシュ RPC・US-A T4 + US-B B3 済）** — [`fetchDashboardOverview.api.ts`](../../src/features/links/api/fetchDashboardOverview.api.ts): `get_dashboard_overview` を `p_tz` 付きで呼び、Zod（`dashboardOverviewRpcSchema`）で `daily_totals`（7 件）、`daily_by_collection`（行ごと `date` / `collection_id` / 件数）、`daily_by_domain` を検証（現状 `daily_by_domain` は `z.array(z.unknown())`。**US-C** で行スキーマ厳格化予定 — [dashboard-overview-us-c §6・C3](./dashboard-overview-us-c.md#us-c-execution-plan)）。開発ビルドでは成功時に `__DEV__` のみ `console.log` でパース後 JSON を出力可能。
...
| 項目                                    | 状態                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| RPC `get_dashboard_overview(p_tz text)` | **実装済み**（`daily_totals` §2 どおり；**`daily_by_collection` は B1 で実データ**（形状は [US-B §3](./dashboard-overview-us-b.md)）；**`daily_by_domain` は US-C C1 で実データ**（形状は [US-C §3](./dashboard-overview-us-c.md#us-c-rpc-contract)）。クライアントの Zod 厳格化は **C3**）                                                                                                                                                                                                                                                                                  |
...
| 検証記録                                | [dashboard-overview-us-a.md](./dashboard-overview-us-a.md) §5（T1〜T9 サマリ）、§6（T6〜T9）；コレクション内訳（B1〜B7）は [dashboard-overview-us-b.md](./dashboard-overview-us-b.md) §5・§6；**US-C**（C1 済・C2〜C7 未）のタスク・DoD は [dashboard-overview-us-c.md](./dashboard-overview-us-c.md) §5・§6                                                                                                                                                                                                                                                                 |
