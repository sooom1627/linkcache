...
- **React Query（US-A T5 済）** — [`useDashboardOverviewQuery`](../../src/features/links/hooks/useDashboardOverviewQuery.ts): `linkQueryKeys.dashboardOverview({ tz })`（`tz` は [`getDeviceTimezone`](../../src/shared/utils/timezone.ts)）で `fetchDashboardOverview` を `useQuery` し、`dashboardOverviewStaleTimeMs`（5 分）を `staleTime` に設定。**データ合成（T6 済）** — [`useDashboardOverviewData`](../../src/features/links/hooks/useDashboardOverviewData.ts) が本フックを呼び、チャート用 `addedByDay` / `readByDay` に `daily_totals` を反映。**mutation の invalidate は T7**。**画面側の loading / エラー合成は T8 / US-X**。検証記録は [dashboard-overview-us-a.md §5](./dashboard-overview-us-a.md#5-実装済みt1t6サマリ)。
...
| React Query（キー・取得フック）         | **実装済み（T5）** — [`linkQueryKeys.dashboardOverview`](../../src/features/links/constants/queryKeys.ts)、[`useDashboardOverviewQuery`](../../src/features/links/hooks/useDashboardOverviewQuery.ts)、テスト [`useDashboardOverviewQuery.test.ts`](../../src/features/links/__tests__/hooks/useDashboardOverviewQuery.test.ts)。**mutation からの invalidate は T7** |
...
| 検証記録                                | [dashboard-overview-us-a.md](./dashboard-overview-us-a.md) §5（T1〜T6 サマリ）                                                                                                                                                                                                                                                                                        |
...
**US-A（ダッシュ概要 RPC）**: [`fetchDashboardOverview.api.ts`](../../src/features/links/api/fetchDashboardOverview.api.ts) を追加済み（`dashboardOverviewRpcSchema`・`fetchDashboardOverview(pTz)`）。検証記録は [dashboard-overview-us-a.md §5](./dashboard-overview-us-a.md#5-実装済みt1t6サマリ)（T4 含む）。
...
- **`linkQueryKeys.dashboardOverview({ tz })`**: `['links','dashboard','overview', { tz }]`。§2 の IANA 暦日と RPC `p_tz` を一致させ、TZ 変更時にキャッシュを分離する。T7 の `invalidateQueries` はプレフィックス `['links','dashboard','overview']` で全 `tz` バリアントをまとめて無効化可能。
- **`useDashboardOverviewQuery`**: [`useDashboardOverviewQuery.ts`](../../src/features/links/hooks/useDashboardOverviewQuery.ts) で `useQuery` + `fetchDashboardOverview(tz)`。`staleTime` は export 定数 **`dashboardOverviewStaleTimeMs`**（5 分）。`retry` はアプリの QueryClient デフォルトに従う（SKILL の数分 `staleTime` と整合）。
