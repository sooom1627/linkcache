...
**実装進捗（要約）**: ユーザーストーリー **US-A**（週次グラフの `daily_totals`・T1〜T9）は **完了**（[dashboard-overview-us-a.md](./dashboard-overview-us-a.md)）。**US-B**（`daily_by_collection`・collection_table・B1〜B7、共有 fixtures と品質ゲート含む）は **完了**（[dashboard-overview-us-b.md](./dashboard-overview-us-b.md)）。**US-C** は **C1〜C7 完了**（RPC `daily_by_domain`・型・Zod・[`useDashboardOverviewData`](../../src/features/links/hooks/useDashboardOverviewData.ts) のドメインピボット・内訳 loading・fixtures Zod 検証・品質ゲート。一次記録: [dashboard-overview-us-c.md](./dashboard-overview-us-c.md)）。レジリエンス・引っ張り更新（**US-X**）は未。
...
- **画面** — [`src/features/links/screens/DashboardOverview.tsx`](../../src/features/links/screens/DashboardOverview.tsx): `useDashboardOverviewData` と `useDashboardOverviewUi` でチャート／内訳を表示。全画面スケルトンは **`dashboardOverviewPending` のみ**（RPC 初回未取得時）。ダッシュ RPC のバックグラウンド再取得中はチャート周りを `opacity` で軽減。**エラー UI・再試行・RefreshControl は未（US-X）**。
- **データ** — [`useDashboardOverviewData.ts`](../../src/features/links/hooks/useDashboardOverviewData.ts): **チャート**は **US-A T6 済** — `daily_totals`。**コレクション内訳**は **US-B B4 済** — `daily_by_collection` を `useCollections` 順でピボット。**ドメイン内訳**は **US-C C4 済** — `daily_by_domain` をピボット（`useLinks` 上限・クライアント mock 行列は撤去済み）。[`fetchDashboardOverview.api.ts`](../../src/features/links/api/fetchDashboardOverview.api.ts) は `daily_totals` / `daily_by_collection` / `daily_by_domain` を Zod 厳格化（**C3 済**）。
- **UI ロジック** — [`useDashboardOverviewUi.tsx`](../../src/features/links/hooks/useDashboardOverviewUi.tsx) および [`useDashboardChartUi`](../../src/features/links/hooks/useDashboardChartUi.tsx) / [`useDashboardBreakdownUi.ts`](../../src/features/links/hooks/useDashboardBreakdownUi.ts): **US-B B5 + US-C C5 済** — コレクションタブの `isTableLoading` は `collectionsLoading || dashboardOverviewPending`、ドメインタブは `dashboardOverviewPending`。テスト [`useDashboardBreakdownUi.loading.test.ts`](../../src/features/links/__tests__/hooks/useDashboardBreakdownUi.loading.test.ts)。
...
**解釈**: **チャート**・**コレクション内訳**・**ドメイン内訳**はいずれも **単一 RPC** `get_dashboard_overview` 由来（US-A / US-B / US-C 完了）。ドメインは `daily_by_domain` をピボットし、`useLinks` 件数上限には依存しない。
...
| データ合成（コレクション内訳・US-B B4）   | **実装済み** — 同フックが `daily_by_collection` を `useCollections` 順でピボット。テスト [`useDashboardOverviewData.test.ts`](../../src/features/links/__tests__/hooks/useDashboardOverviewData.test.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| データ合成（ドメイン内訳・US-C C4）       | **実装済み** — 同フックが `daily_by_domain` をピボットし `domainAddedStatsByDay` / `domainReadStatsByDay` / `domainStats` を生成。テスト同上                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 内訳テーブル loading（US-B B5 + US-C C5） | **実装済み** — [`useDashboardBreakdownUi.ts`](../../src/features/links/hooks/useDashboardBreakdownUi.ts): コレクションは `collectionsLoading` または `dashboardOverviewPending`、ドメインは `dashboardOverviewPending`。テスト [`useDashboardBreakdownUi.loading.test.ts`](../../src/features/links/__tests__/hooks/useDashboardBreakdownUi.loading.test.ts)                                                                                                                                                                                                                                                                                                                                                                                               |
...
| 検証記録                                  | [dashboard-overview-us-a.md](./dashboard-overview-us-a.md) §5・§6；コレクション内訳は [dashboard-overview-us-b.md](./dashboard-overview-us-b.md) §5・§6；ドメイン内訳は [dashboard-overview-us-c.md](./dashboard-overview-us-c.md) §5・§6（**C1〜C7 完了**）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
- **ドメイン内訳（US-C C4 済）**: `daily_by_domain` をピボットした `domainAddedStatsByDay` / `domainReadStatsByDay` / `domainStats`。クライアント側の `mockAddedByDay` / `mockReadByDay` / `splitDayTotalAcrossBuckets` 依存は撤去済み。
- **コレクション一覧メタ**: 表示名・絵文字等は引き続き `useCollections`。集計数字は RPC 由来（上記）。**同じ画面で二重フェッチ**のトレードオフは従来どおり、必要なら RPC 一本化を検討（ドキュメント化）。
- **ローディング合成（T8 + US-C）**: [`DashboardOverview.tsx`](../../src/features/links/screens/DashboardOverview.tsx) の全画面スケルトンは `dashboardOverviewPending` のみ。`dashboardOverviewFetching && !dashboardOverviewPending` のときチャートを `opacity` で軽減。
- **内訳テーブル loading（US-B B5 + US-C C5 済）**: [`useDashboardBreakdownUi`](../../src/features/links/hooks/useDashboardBreakdownUi.ts) — コレクション `collectionsLoading || dashboardOverviewPending`、ドメイン `dashboardOverviewPending`。
