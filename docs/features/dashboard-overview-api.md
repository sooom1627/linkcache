...
- **React Query（US-A T5 済）** — [`useDashboardOverviewQuery`](../../src/features/links/hooks/useDashboardOverviewQuery.ts): `linkQueryKeys.dashboardOverview({ tz })`（`tz` は [`getDeviceTimezone`](../../src/shared/utils/timezone.ts)）で `fetchDashboardOverview` を `useQuery` し、`dashboardOverviewStaleTimeMs`（5 分）を `staleTime` に設定。**`useDashboardOverviewData` / 画面とは未配線**（T6 以降）。検証記録は [dashboard-overview-us-a.md §5](./dashboard-overview-us-a.md#5-実装済みt1t5サマリ)。
...
| 検証記録                                | [dashboard-overview-us-a.md](./dashboard-overview-us-a.md) §5（T1〜T5 サマリ）                                                                                                                                                                                                                                                                                        |
...
**US-A（ダッシュ概要 RPC）**: [`fetchDashboardOverview.api.ts`](../../src/features/links/api/fetchDashboardOverview.api.ts) を追加済み（`dashboardOverviewRpcSchema`・`fetchDashboardOverview(pTz)`）。検証記録は [dashboard-overview-us-a.md §5](./dashboard-overview-us-a.md#5-実装済みt1t5サマリ)（T4 含む）。
...
- **React Query フック（T5）**: [`useDashboardOverviewQuery.test.ts`](../../src/features/links/__tests__/hooks/useDashboardOverviewQuery.test.ts) — `fetchDashboardOverview` をモックし、TZ 引数・`linkQueryKeys.dashboardOverview` とのキャッシュ一致・エラー状態・`staleTime` を検証。T6 で `useDashboardOverviewData` を接続したら、必要に応じてデータ層・fixtures を拡張（§8 下段）。
