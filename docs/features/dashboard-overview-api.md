...
**Git 履歴の注意**: コミット `7a3d917` のメッセージは *「remove unused useDashboardBreakdownUi hook」* だが、実際の変更は **`useDashboardBreakdownUi` を `.tsx` から `.ts` にリネームし実装を簡略化したもの**であり、フックは削除されていない。ドキュメントやレビューで当該コミットを根拠に「フックが消えた」と解釈しないこと。
...
- **API 層**: 既存パターンに倣い [`src/features/links/__tests__/api/`](../../src/features/links/__tests__/api/)に Zod 成功／失敗・認証エラーのテスト。新規 `fetchDashboardOverview.api.ts` なら同ディレクトリに `fetchDashboardOverview.test.ts`（命名は既存に合わせる）。
