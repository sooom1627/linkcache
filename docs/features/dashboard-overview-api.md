...
**実装進捗（要約）**: ユーザーストーリー **US-A**（週次グラフの `daily_totals`・T1〜T9）は **完了**（[dashboard-overview-us-a.md](./dashboard-overview-us-a.md)）。**US-B の B1〜B5**（上記に加え [`useDashboardBreakdownUi`](../../src/features/links/hooks/useDashboardBreakdownUi.ts) でコレクション内訳テーブルの `isTableLoading` に `dashboardOverviewPending` を合成）は **完了**（[dashboard-overview-us-b.md](./dashboard-overview-us-b.md)）。**US-B の B6〜B7**（fixtures 強化・ストーリー締め）および **US-C**（ドメイン内訳）、レジリエンス・引っ張り更新（**US-X**）は未。
