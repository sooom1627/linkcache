...
**実装進捗（要約）**: ユーザーストーリー **US-A**（週次グラフの `daily_totals`・T1〜T9）は **完了**（[dashboard-overview-us-a.md](./dashboard-overview-us-a.md)）。**US-B**（`daily_by_collection`・collection_table・B1〜B7、共有 fixtures と品質ゲート含む）は **完了**（[dashboard-overview-us-b.md](./dashboard-overview-us-b.md)）。**US-C** は **C1 完了**（RPC `daily_by_domain`・[`20260322103614_get_dashboard_overview_daily_by_domain.sql`](../../supabase/migrations/20260322103614_get_dashboard_overview_daily_by_domain.sql)）、**C2・C3 完了**（[`supabase.types.ts`](../../src/features/links/types/supabase.types.ts) の `DashboardOverviewRpcJson`、`fetchDashboardOverview` の Zod）；**C4〜C7**（フック置換・fixtures 等）は未（[dashboard-overview-us-c.md](./dashboard-overview-us-c.md)）。レジリエンス・引っ張り更新（**US-X**）は未。
...
- **画面** — [`src/features/links/screens/DashboardOverview.tsx`](../../src/features/links/screens/DashboardOverview.tsx): `useDashboardOverviewData` と `useDashboardOverviewUi` でチャート／内訳を表示。スケルトンは `collectionsLoading || domainsLoading || dashboardOverviewPending`（**US-A T8 済**）。ダッシュ RPC のバックグラウンド再取得中はチャート周りを `opacity` で軽減。**エラー UI・再試行・RefreshControl は未（US-X）**。
- **データ** — [`useDashboardOverviewData.ts`](../../src/features/links/hooks/useDashboardOverviewData.ts): **チャートの週次系列**（`addedByDay` / `readByDay`）は **US-A T6 済** — [`useDashboardOverviewQuery`](../../src/features/links/hooks/useDashboardOverviewQuery.ts) 経由で RPC の `daily_totals` を使用。未取得時は 7 日ゼロプレースホルダ。**コレクション内訳**（`collectionAddedStatsByDay` / `collectionReadStatsByDay` / `collectionStats`）は **US-B B4 済** — RPC の `daily_by_collection` を `useCollections` の並びでピボット（疎行列は 0 埋め）。[`fetchDashboardOverview.api.ts`](../../src/features/links/api/fetchDashboardOverview.api.ts) は **B3 済** — `daily_by_collection` 行を Zod で厳格化。**コレクション一覧メタ**は `useCollections`、**ドメイン集計**は引き続き `useLinks({ limit: 500 })` と [`buildDomainStatsFromLinks`](../../src/features/links/utils/dashboardStats.ts)。**ドメインの日別行列**は [`mockAddedByDay` / `mockReadByDay`](../../src/features/links/utils/dashboardStats.ts) ＋ `splitDayTotalAcrossBuckets`（**US-C** でサーバ系列へ）。
- **UI ロジック** — [`useDashboardOverviewUi.tsx`](../../src/features/links/hooks/useDashboardOverviewUi.tsx) および [`useDashboardChartUi`](../../src/features/links/hooks/useDashboardChartUi.tsx) / [`useDashboardBreakdownUi.ts`](../../src/features/links/hooks/useDashboardBreakdownUi.ts): 入力は `DashboardOverviewData` 形状に依存。**US-B B5 済** — コレクション内訳の `isTableLoading` は `collectionsLoading` に加え `dashboardOverviewPending` を合成（ドメインタブは `domainsLoading` のみ）。**US-C** でドメインタブにも `dashboardOverviewPending` を合成する予定（[dashboard-overview-us-c.md §6・C5](./dashboard-overview-us-c.md#us-c-execution-plan)）。テスト [`useDashboardBreakdownUi.test.ts`](../../src/features/links/__tests__/hooks/useDashboardBreakdownUi.test.ts)。
- **テスト fixtures（US-B B6 済）** — [`dashboardOverview.fixtures.ts`](../../src/features/links/testing/dashboardOverview.fixtures.ts) に RPC 成功ペイロードと `createMinimalOverviewData` を集約。[`dashboardOverview.fixtures.test.ts`](../../src/features/links/testing/__tests__/dashboardOverview.fixtures.test.ts) で `dashboardOverviewRpcSchema` と整合（`collection_id` は Zod `.uuid()` 互換の RFC 4122 形式）。
...
**解釈**: **チャートの週次 added/read 系列**は T6 で RPC に接続済み。**コレクション内訳・`collectionStats`** は **US-B B4** で `daily_by_collection` に接続済み。**ドメイン**の日別行列と `domainStats` は **US-C** までモック／`useLinks` 上限依存のまま。`useLinks` の 500 件上限によるドメイン集計は**母集団を代表しない**可能性が引き続きある。

- **API（ダッシュ RPC・US-A T4 + US-B B3 + US-C C3 済）** — [`fetchDashboardOverview.api.ts`](../../src/features/links/api/fetchDashboardOverview.api.ts): `get_dashboard_overview` を `p_tz` 付きで呼び、Zod（`dashboardOverviewRpcSchema`）で `daily_totals`（7 件）、`daily_by_collection`（行ごと `date` / `collection_id` / 件数）、`daily_by_domain`（行ごと `date` / `domain` / 件数、`.strict()`）を検証（[dashboard-overview-us-c §6・C3](./dashboard-overview-us-c.md#us-c-execution-plan)）。開発ビルドでは成功時に `__DEV__` のみ `console.log` でパース後 JSON を出力可能。
- **React Query（US-A T5・T7 済）** — [`useDashboardOverviewQuery`](../../src/features/links/hooks/useDashboardOverviewQuery.ts): `linkQueryKeys.dashboardOverview({ tz })`（`tz` は [`getDeviceTimezone`](../../src/shared/utils/timezone.ts)）で `fetchDashboardOverview` を `useQuery` し、`dashboardOverviewStaleTimeMs`（30 分）を `staleTime` に設定。戻り値に **`isPending` / `isFetching`** を含む（T8 で画面に透過）。**データ合成（T6 済）** — [`useDashboardOverviewData`](../../src/features/links/hooks/useDashboardOverviewData.ts) が本フックを呼び、チャート用 `addedByDay` / `readByDay` に `daily_totals` を反映し、**`dashboardOverviewPending` / `dashboardOverviewFetching`** を画面へ渡す。**mutation の invalidate（T7 済）** — 9 本の mutation が [`linkQueryKeys.dashboardOverviewPrefix()`](../../src/features/links/constants/queryKeys.ts) でプレフィックス無効化。**画面 loading・週 0 の空コピーは T8 済**。**エラー UI・引っ張り更新は US-X**。**US-A（T1〜T9）は完了**（品質ゲート・[dashboard-overview-us-a.md §6 T9](./dashboard-overview-us-a.md#t9--回帰テスト品質ゲート)）。検証記録は [dashboard-overview-us-a.md §5](./dashboard-overview-us-a.md#5-実装済みt1t9サマリ)。
...
- **ドメイン内訳の母集団**: **`daily_totals` と同一**。`link_status` の added は `created_at` の日バケット、read は `read_at` の日バケット（いずれも **全 `triage_status`**）。ドメインキーは `links.url` から `extract_domain_for_dashboard` 相当で抽出し、チャート合計とドメイン行の ADDED／READ の**週次合計が一致**する。
...
- **推奨アウトプット形（例）**
  ...
  - `daily_by_collection`: 同期間・コレクション別の **フラット配列** `{ date, collection_id, added_count, read_count }[]`（`date` は `YYYY-MM-DD`、`collection_id` は UUID 文字列、**両方 0 の行は省略（疎行列）**）— **B1 で RPC 確定**（[dashboard-overview-us-b.md §3](./dashboard-overview-us-b.md)）
  - `daily_by_domain`: ドメイン抽出を SQL 関数化し、アプリの [`extractDomain`](../../src/features/links/utils/urlUtils.ts) と**同じ入力に対して同じ文字列**になること。**added/read の母集団は `daily_totals` と同じ**（§2）
...
| 項目                                    | 状態                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| RPC `get_dashboard_overview(p_tz text)` | **実装済み**（`daily_totals` §2 どおり；**`daily_by_collection` は B1 で実データ**（形状は [US-B §3](./dashboard-overview-us-b.md)）；**`daily_by_domain` は US-C C1 で実データ**（形状は [US-C §3](./dashboard-overview-us-c.md#us-c-rpc-contract)）。クライアントの Zod 厳格化は **C3 済**（`dashboardDailyByDomainRowSchema`）                                                                                                                                                                                                                                                                                                                                                                                                                          |
| リポジトリ DDL                          | 起点 [`20260322000000_get_dashboard_overview.sql`](../../supabase/migrations/20260322000000_get_dashboard_overview.sql) → [`20260322032918`](../../supabase/migrations/20260322032918_dashboard_overview_seven_day_timestamp_filter.sql) → [`20260322080212`](../../supabase/migrations/20260322080212_get_dashboard_overview_daily_by_collection_current_membership_semantics.sql) → **`daily_by_domain`**: [`20260322103614_get_dashboard_overview_daily_by_domain.sql`](../../supabase/migrations/20260322103614_get_dashboard_overview_daily_by_domain.sql) → **母集団を `daily_totals` に揃える**: [`20260323120000_dashboard_overview_domain_align_totals.sql`](../../supabase/migrations/20260323120000_dashboard_overview_domain_align_totals.sql) |
...
| React Query（キー・取得・invalidate）   | **実装済み（T5・T7）** — [`linkQueryKeys.dashboardOverview`](../../src/features/links/constants/queryKeys.ts)、[`dashboardOverviewPrefix()`](../../src/features/links/constants/queryKeys.ts)（mutation 無効化用）、[`useDashboardOverviewQuery`](../../src/features/links/hooks/useDashboardOverviewQuery.ts)（`isPending` / `isFetching` 含む）、テスト [`useDashboardOverviewQuery.test.ts`](../../src/features/links/__tests__/hooks/useDashboardOverviewQuery.test.ts)。リンク／コレクション系 mutation 9 本がダッシュ概要を `invalidate`（本書 **§5** の mutation 表）                                                                                                                                                                               |
| データ合成（チャート・T6）              | **実装済み** — 同上フックが `daily_totals` をチャート系列にマップし **`dashboardOverviewPending` / `dashboardOverviewFetching`** を返す。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| データ合成（コレクション内訳・US-B B4） | **実装済み** — 同フックが `daily_by_collection` を `useCollections` 順でピボットし `collectionAddedStatsByDay` / `collectionReadStatsByDay` / `collectionStats` を生成。ドメイン行列はモック継続（US-C）。テスト [`useDashboardOverviewData.test.ts`](../../src/features/links/__tests__/hooks/useDashboardOverviewData.test.ts)                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 内訳テーブル loading（US-B B5）         | **実装済み** — [`useDashboardBreakdownUi.ts`](../../src/features/links/hooks/useDashboardBreakdownUi.ts) が `tableView === "collection"` のとき `isTableLoading` に `dashboardOverviewPending` を含める。テスト [`useDashboardBreakdownUi.test.ts`](../../src/features/links/__tests__/hooks/useDashboardBreakdownUi.test.ts)                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 画面・チャート UI（T8）                 | **実装済み** — [`DashboardOverview.tsx`](../../src/features/links/screens/DashboardOverview.tsx)、[`useDashboardChartUi`](../../src/features/links/hooks/useDashboardChartUi.tsx)（`showEmptyWeekHint`）、[`DashboardWeeklyActivityChart.tsx`](../../src/features/links/components/dashboard/DashboardWeeklyActivityChart.tsx)、i18n `chart_week_empty_hint`、[`useDashboardChartUi.test.ts`](../../src/features/links/__tests__/hooks/useDashboardChartUi.test.ts)                                                                                                                                                                                                                                                                                        |
| 品質ゲート（T9）                        | **完了** — `pnpm test` / `pnpm run check`（[dashboard-overview-us-a.md §6 T9](./dashboard-overview-us-a.md#t9--回帰テスト品質ゲート)）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 検証記録                                | [dashboard-overview-us-a.md](./dashboard-overview-us-a.md) §5（T1〜T9 サマリ）、§6（T6〜T9）；コレクション内訳（B1〜B7）は [dashboard-overview-us-b.md](./dashboard-overview-us-b.md) §5・§6；**US-C**（C1〜C3 済・C4〜C7 未）のタスク・DoD は [dashboard-overview-us-c.md](./dashboard-overview-us-c.md) §5・§6                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| **往復・再取得**         | **1 RPC**（§3.1）に加え、React Query では **`staleTime` を 30 分**などにし、タブ復帰での無駄な refetch を抑える。§5 の **invalidate が多箇所から飛ぶ**ため、開発中は Network タブで「操作 1 回あたりのダッシュ再取得回数」が過剰でないか確認する。 |
...
**US-A（ダッシュ概要 RPC）**: [`fetchDashboardOverview.api.ts`](../../src/features/links/api/fetchDashboardOverview.api.ts) を追加済み（`dashboardOverviewRpcSchema`・`fetchDashboardOverview(pTz)`）。**ストーリー US-A は T9 まで完了**（[dashboard-overview-us-a.md](./dashboard-overview-us-a.md) ストーリーステータス・§5〜§6）。検証記録は [dashboard-overview-us-a.md §5](./dashboard-overview-us-a.md#5-実装済みt1t9サマリ)（T4 含む）。
...
[`src/features/links/constants/queryKeys.ts`](../../src/features/links/constants/queryKeys.ts)を拡張済み（T5）。

- **`linkQueryKeys.dashboardOverview({ tz })`**: `['links','dashboard','overview', { tz }]`。§2 の IANA 暦日と RPC `p_tz` を一致させ、TZ 変更時にキャッシュを分離する。
- **`linkQueryKeys.dashboardOverviewPrefix()`**: `['links','dashboard','overview']`。**T7 済** — 上記プレフィックスで `invalidateQueries` し、全 `tz` バリアントをまとめて無効化する。
- **`useDashboardOverviewQuery`**: [`useDashboardOverviewQuery.ts`](../../src/features/links/hooks/useDashboardOverviewQuery.ts) で `useQuery` + `fetchDashboardOverview(tz)`。`staleTime` は export 定数 **`dashboardOverviewStaleTimeMs`**（30 分）。`retry` はアプリの QueryClient デフォルトに従う。戻り値に **`isPending` / `isFetching`**（画面のスケルトン・再取得表示用、**T8**）。鮮度は mutation の `invalidate`（T7 済）と組み合わせる。
- **無効化（invalidate・T7 実装済み）**: 次表の各 mutation から `dashboardOverviewPrefix()` を無効化に使用。**新規 mutation フック**を追加したら、同パターンでダッシュキーも無効化する（API ファイルそのものではなく、**mutation を持つフック**が実際の無効化箇所）。
...
- **US-A T6 済（チャート）**: `addedByDay` / `readByDay` は [`useDashboardOverviewQuery`](../../src/features/links/hooks/useDashboardOverviewQuery.ts) の `daily_totals` 由来。`data` 未確定時は 7 日ゼロ。
- **US-B B4 済（コレクション内訳）**: `collectionAddedStatsByDay` / `collectionReadStatsByDay` / `collectionStats` は RPC の `daily_by_collection` を **`useCollections` の id・並び**でピボットした結果（未取得時はコレクションごと 0）。`itemsCount` および仮 `readCount`（`Math.floor(n * 0.45)`）はコレクションサマリから撤去済み。
- **削除対象（未・US-C）**: **ドメイン**の日別行列についての `mockAddedByDay` / `mockReadByDay` 依存、`splitDayTotalAcrossBuckets` による仮日別行列（`daily_by_domain` 実装後にサーバ系列へ）。**実装手順・受け入れ**は [dashboard-overview-us-c.md](./dashboard-overview-us-c.md)（[§6・C4](./dashboard-overview-us-c.md#us-c-execution-plan)）。
- **コレクション一覧メタ**: 表示名・絵文字等は引き続き `useCollections`。集計数字は RPC 由来（上記）。**同じ画面で二重フェッチ**のトレードオフは従来どおり、必要なら RPC 一本化を検討（ドキュメント化）。
- **ドメイン**: RPC のドメイン集計に寄せ、**§2 の母集団（`new` / `done` のみ）**を SQL で再現する。クライアント [`buildDomainStatsFromLinks`](../../src/features/links/utils/dashboardStats.ts) は **US-C** で削除またはフォールバック廃止を検討（[dashboard-overview-us-c.md](./dashboard-overview-us-c.md)）。
- **ローディング合成（T8 済）**: [`DashboardOverview.tsx`](../../src/features/links/screens/DashboardOverview.tsx) で `collectionsLoading || domainsLoading || dashboardOverviewPending` をスケルトンに使用。`dashboardOverviewFetching && !dashboardOverviewPending` のときチャートを `opacity` で軽減。
- **内訳テーブル loading（US-B B5 済）**: [`useDashboardBreakdownUi`](../../src/features/links/hooks/useDashboardBreakdownUi.ts) でコレクションタブ時の `isTableLoading` に `dashboardOverviewPending` を合成（画面が部分表示に変わっても RPC 未確定の誤表示を防ぐ）。
...
3. **空データ**: **チャートの週 7 日が added/read とも 0 のとき** — **US-A T8 済**（[`useDashboardChartUi`](../../src/features/links/hooks/useDashboardChartUi.tsx) の `showEmptyWeekHint`、`links.dashboard.chart_week_empty_hint`）。内訳テーブル側の空コピー強化は US-B/C と US-X で要否を判断。
...
- **React Query フック（T5・T8 追記）**: [`useDashboardOverviewQuery.test.ts`](../../src/features/links/__tests__/hooks/useDashboardOverviewQuery.test.ts) — `fetchDashboardOverview` をモックし、TZ 引数・`linkQueryKeys.dashboardOverview` とのキャッシュ一致・エラー状態・`staleTime`・**`isPending` / `isFetching`** を検証。
- **データ合成フック（T6・T8・US-B B4 追記）**: [`useDashboardOverviewData.test.ts`](../../src/features/links/__tests__/hooks/useDashboardOverviewData.test.ts) — `useDashboardOverviewQuery` 等を境界モックし、`daily_totals` とチャート系列の一致、**コレクション**は固定 `daily_by_collection` のピボット・`collectionStats`、**ドメイン**は mock 系列維持、overview pending 時のコレクションゼロ行列、未取得時ゼロ、**`dashboardOverviewPending` / `dashboardOverviewFetching`** を検証。fixtures の共通デフォルトは [`dashboardOverview.fixtures.ts`](../../src/features/links/testing/dashboardOverview.fixtures.ts)（`dashboardOverviewPending` / `dashboardOverviewFetching` のデフォルト含む）。
