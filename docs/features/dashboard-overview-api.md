...
- **React Query（US-A T5・T7 済）** — [`useDashboardOverviewQuery`](../../src/features/links/hooks/useDashboardOverviewQuery.ts): `linkQueryKeys.dashboardOverview({ tz })`（`tz` は [`getDeviceTimezone`](../../src/shared/utils/timezone.ts)）で `fetchDashboardOverview` を `useQuery` し、`dashboardOverviewStaleTimeMs`（30 分）を `staleTime` に設定。**データ合成（T6 済）** — [`useDashboardOverviewData`](../../src/features/links/hooks/useDashboardOverviewData.ts) が本フックを呼び、チャート用 `addedByDay` / `readByDay` に `daily_totals` を反映。**mutation の invalidate（T7 済）** — 9 本の mutation が [`linkQueryKeys.dashboardOverviewPrefix()`](../../src/features/links/constants/queryKeys.ts) でプレフィックス無効化。**画面側の loading / エラー合成は T8 / US-X**。検証記録は [dashboard-overview-us-a.md §5](./dashboard-overview-us-a.md#5-実装済みt1t7サマリ)。
...
| React Query（キー・取得・invalidate）   | **実装済み（T5・T7）** — [`linkQueryKeys.dashboardOverview`](../../src/features/links/constants/queryKeys.ts)、[`dashboardOverviewPrefix()`](../../src/features/links/constants/queryKeys.ts)（mutation 無効化用）、[`useDashboardOverviewQuery`](../../src/features/links/hooks/useDashboardOverviewQuery.ts)、テスト [`useDashboardOverviewQuery.test.ts`](../../src/features/links/__tests__/hooks/useDashboardOverviewQuery.test.ts)。リンク／コレクション系 mutation 9 本がダッシュ概要を `invalidate`（本書 **§5** の mutation 表） |
| データ合成（チャート系列・T6）          | **実装済み** — [`useDashboardOverviewData`](../../src/features/links/hooks/useDashboardOverviewData.ts) が `daily_totals` をチャート系列にマップ、テスト [`useDashboardOverviewData.test.ts`](../../src/features/links/__tests__/hooks/useDashboardOverviewData.test.ts)。内訳日別はモック継続（US-B/C）                                                                                                                                                                                                                                  |
| 検証記録                                | [dashboard-overview-us-a.md](./dashboard-overview-us-a.md) §5（T1〜T7 サマリ）、§6 T7                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
**US-A（ダッシュ概要 RPC）**: [`fetchDashboardOverview.api.ts`](../../src/features/links/api/fetchDashboardOverview.api.ts) を追加済み（`dashboardOverviewRpcSchema`・`fetchDashboardOverview(pTz)`）。検証記録は [dashboard-overview-us-a.md §5](./dashboard-overview-us-a.md#5-実装済みt1t7サマリ)（T4 含む）。
...
- **`useDashboardOverviewQuery`**: [`useDashboardOverviewQuery.ts`](../../src/features/links/hooks/useDashboardOverviewQuery.ts) で `useQuery` + `fetchDashboardOverview(tz)`。`staleTime` は export 定数 **`dashboardOverviewStaleTimeMs`**（30 分）。`retry` はアプリの QueryClient デフォルトに従う。鮮度は mutation の `invalidate`（T7 済）と組み合わせる。
...
- **React Query フック（T5）**: [`useDashboardOverviewQuery.test.ts`](../../src/features/links/__tests__/hooks/useDashboardOverviewQuery.test.ts) — `fetchDashboardOverview` をモックし、TZ 引数・`linkQueryKeys.dashboardOverview` とのキャッシュ一致・エラー状態・`staleTime` を検証。
- **データ合成フック（T6）**: [`useDashboardOverviewData.test.ts`](../../src/features/links/__tests__/hooks/useDashboardOverviewData.test.ts) — `useDashboardOverviewQuery` 等を境界モックし、`daily_totals` とチャート系列の一致・内訳モック維持・未取得時ゼロを検証。fixtures の共通デフォルトは [`dashboardOverview.fixtures.ts`](../../src/features/links/testing/dashboardOverview.fixtures.ts)（§8 下段）。
