...
**実装進捗（要約）**: ユーザーストーリー **US-A**（週次グラフの `daily_totals`・T1〜T9）は **完了**（[dashboard-overview-us-a.md](./dashboard-overview-us-a.md)）。**US-B**（`daily_by_collection`・collection_table・B1〜B7、共有 fixtures と品質ゲート含む）は **完了**（[dashboard-overview-us-b.md](./dashboard-overview-us-b.md)）。**US-C**（ドメイン内訳）、レジリエンス・引っ張り更新（**US-X**）は未。
...
| 項目                                    | 状態                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| RPC `get_dashboard_overview(p_tz text)` | **実装済み**（`daily_totals` §2 どおり；**`daily_by_collection` は B1 で実データ**（形状は [US-B §3](./dashboard-overview-us-b.md)）；`daily_by_domain` は当面 `[]`）                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| 検証記録                                | [dashboard-overview-us-a.md](./dashboard-overview-us-a.md) §5（T1〜T9 サマリ）、§6（T6〜T9）；コレクション内訳（B1〜B5）は [dashboard-overview-us-b.md](./dashboard-overview-us-b.md) §5・§6                                                                                                                                                                                                                                                                                                                                                                                 |
...
- **削除対象（未・US-C）**: **ドメイン**の日別行列についての `mockAddedByDay` / `mockReadByDay` 依存、`splitDayTotalAcrossBuckets` による仮日別行列（`daily_by_domain` 実装後にサーバ系列へ）。
- **コレクション一覧メタ**: 表示名・絵文字等は引き続き `useCollections`。集計数字は RPC 由来（上記）。**同じ画面で二重フェッチ**のトレードオフは従来どおり、必要なら RPC 一本化を検討（ドキュメント化）。
- **ドメイン**: RPC のドメイン集計に寄せ、**§2 の母集団（`new` / `done` のみ）**を SQL で再現する。クライアント [`buildDomainStatsFromLinks`](../../src/features/links/utils/dashboardStats.ts) は **US-C** で削除またはフォールバック廃止を検討。
