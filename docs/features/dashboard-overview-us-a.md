...
| **T1〜T4 実装**                      | **完了**（T1〜T3: [§5](#5-実装検証記録t1t3) · T4: [§5.1](#51-実装検証記録t4)） |
...
**実施日**: 2026-03-22（リポジトリ現状に基づく）

| 検証項目                                                                                                                                                                                                                                                              | 結果                                                                                                                                                                                                                                    |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`fetchDashboardOverview.api.ts`](../../src/features/links/api/fetchDashboardOverview.api.ts) — `get_dashboard_overview` + `p_tz`、認証は [`fetchCollections.api.ts`](../../src/features/links/api/fetchCollections.api.ts) と同型（`getUser` → `Not authenticated`） | OK                                                                                                                                                                                                                                      |
| Zod `dashboardOverviewRpcSchema` — `daily_totals` 長さ 7、`date` / `added_count` / `read_count`、`daily_by_*` は配列                                                                                                                                                  | OK                                                                                                                                                                                                                                      |
| 失敗時メッセージ — `Not authenticated` / RPC `error.message` / `Validation failed: …`                                                                                                                                                                                 | OK（既存 API と整合）                                                                                                                                                                                                                   |
| [`fetchDashboardOverview.api.test.ts`](../../src/features/links/__tests__/api/fetchDashboardOverview.api.test.ts) — 正常系・未認証・RPC エラー・データなし・Zod 不整合                                                                                                | OK（Jest 緑）                                                                                                                                                                                                                           |
| 静的品質                                                                                                                                                                                                                                                              | OK — `pnpm run check` 通過（実施時点）                                                                                                                                                                                                  |
| MCP `get_logs`（`postgres` / `api`）— ERROR / FATAL の有無                                                                                                                                                                                                            | 実施時点で postgres は LOG のみ。Jest は Supabase をモックのためリモート RPC は未呼び出し                                                                                                                                               |
| 実データ整合性 — `execute_sql` で `SELECT get_dashboard_overview(...)`                                                                                                                                                                                                | `auth.uid()` が無いセッションでは `Not authenticated` で失敗（想定どおり）。アプリ側は **`__DEV__` 時のみ** `console.log('[fetchDashboardOverview]', …)` でパース後 JSON を出力（実機ではログイン後に T5 接続または一時呼び出しで確認） |

**注意**: `links/api` 全体の `getUser()` スタイル統一は [GitHub issue #104](https://github.com/sooom1627/linkcache/issues/104) で別途追跡。
...
- [x] `supabase.rpc('get_dashboard_overview', { p_tz })` を呼ぶ
- [x] Zod で `daily_totals`（7 件・`date` / `added_count` / `read_count`）を parse する
- [x] 失敗時は既存の API エラーパターンに合わせる
- [x] 正常系・異常系（型不整合など）をテストでカバーする

**DoD**: 新規 API テストが緑。`fetchDashboardOverview` が `daily_totals` を返す。→ **満たした**（§5.1）。
...
| 済（T4）     | [`src/features/links/api/fetchDashboardOverview.api.ts`](../../src/features/links/api/fetchDashboardOverview.api.ts)                                              |
| 新規         | `src/features/links/hooks/useDashboardOverviewQuery.ts`（仮称・統合可）                                                                                           |
| 済（T4）     | [`src/features/links/__tests__/api/fetchDashboardOverview.api.test.ts`](../../src/features/links/__tests__/api/fetchDashboardOverview.api.test.ts)                |
...
**進捗メモ**: **T1〜T4**（DB・型・API 層＋ API 単体テスト）は完了（§5・§5.1）。**US-A 全体の Story DoD** には **T5 以降**（React Query・`useDashboardOverviewData` 接続・画面・invalidate など）がまだ必要。
