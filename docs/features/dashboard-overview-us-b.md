...
- [x] ドメイン表示時は従来どおり `domainsLoading` のみ（または仕様どおり維持）

**DoD**: コレクションタブでダッシュ pending 中はテーブルが loading 扱い。実機で確認。 — **B5 完了**（`pnpm test` / `pnpm run check` 通過・[`useDashboardBreakdownUi.test.ts`](../../src/features/links/__tests__/hooks/useDashboardBreakdownUi.test.ts) で振る舞い固定）
...
| 種別                    | パス・備考                                                                                                                                                                                                                                                                                                                                                |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **主に変更**            | **B1〜B5 済**: 上記に加え [`useDashboardBreakdownUi.ts`](../../src/features/links/hooks/useDashboardBreakdownUi.ts)、[`useDashboardBreakdownUi.test.ts`](../../src/features/links/__tests__/hooks/useDashboardBreakdownUi.test.ts)。**B6〜B7 予定**: [`dashboardOverview.fixtures.ts`](../../src/features/links/testing/dashboardOverview.fixtures.ts) 等 |
