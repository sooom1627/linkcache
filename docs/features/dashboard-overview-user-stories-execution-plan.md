...
**進捗サマリ**: **US-A（グラフ／`daily_totals`）は完了**（T1〜T9・2026-03-22 締め）。実装・検証の一次記録は [dashboard-overview-us-a.md](./dashboard-overview-us-a.md)（ストーリーステータス・§5〜§8）。**US-B** は **B1〜B7 完了**（[`dashboardOverview.fixtures.ts`](../../src/features/links/testing/dashboardOverview.fixtures.ts)・品質ゲート含む）。詳細は [dashboard-overview-us-b.md](./dashboard-overview-us-b.md) §5・§6。**US-C** は **C1〜C7 完了**。一次記録は [dashboard-overview-us-c.md](./dashboard-overview-us-c.md)。
...
| **US-B** | collection_table（コレクション別内訳）**— 完了**（B1〜B7） | [`DashboardBreakdownSection`](../../src/features/links/components/dashboard/DashboardBreakdownSection.tsx) で `tableView === "collection"`、`daily_by_collection`、§2 の重複計上。実行タスク一覧: [dashboard-overview-us-b.md](./dashboard-overview-us-b.md) | §2 コレクション、§3.2、§6                                 |
| **US-C** | domain_table（ドメイン別内訳）**— C1〜C7 完了**            | 同セクションで `tableView === "domain"`、`daily_by_domain` を [`useDashboardOverviewData`](../../src/features/links/hooks/useDashboardOverviewData.ts) がピボット。実行タスク一覧: [dashboard-overview-us-c.md](./dashboard-overview-us-c.md)                | §2, §3.2, §6 · [us-c.md](./dashboard-overview-us-c.md)    |
...
```text
supabase/migrations/
  └── …get_dashboard_overview…sql              # 例: `20260322074231_get_dashboard_overview_daily_by_collection.sql`（`daily_by_collection`・US-B B1）
...
src/features/links/
  ...
  ├── testing/
  │   ├── dashboardOverview.fixtures.ts          # T8 + US-B B6: RPC フィクスチャ・`createMinimalOverviewData`（§8）
  │   └── __tests__/dashboardOverview.fixtures.test.ts  # US-B B6: `dashboardOverviewRpcSchema` 整合
  ├── utils/
...
- [x] **US-B B4**: 同フックの **コレクション** `collectionAddedStatsByDay` / `collectionReadStatsByDay` / `collectionStats` を `daily_by_collection` に接続
- [x] `useDashboardOverviewData` から **ドメイン内訳用** `mockAddedByDay` / `mockReadByDay` / `splitDayTotalAcrossBuckets` を除去（[§6](./dashboard-overview-api.md)・**US-C** 完了）
...
- [x] **B6 — fixtures**: [`dashboardOverview.fixtures.ts`](../../src/features/links/testing/dashboardOverview.fixtures.ts)、[`dashboardOverview.fixtures.test.ts`](../../src/features/links/testing/__tests__/dashboardOverview.fixtures.test.ts)（Zod 契約）
- [x] **B7 — 品質ゲート**: `pnpm test` / `pnpm run check`（[US-B B7](./dashboard-overview-us-b.md)）

### US-C：domain_table（`daily_by_domain`）

一次記録（タスク C1〜C7・DoD・RPC 契約）: [dashboard-overview-us-c.md](./dashboard-overview-us-c.md)。

- [x] **C1 — Supabase**: `daily_by_domain` を RPC で返す；母集団は `daily_totals` と同一（全 `triage_status`）、`extract_domain_for_dashboard` と [`extractDomain`](../../src/features/links/utils/urlUtils.ts) 同値、**N=15**・`__other__`（[US-C §3](./dashboard-overview-us-c.md#us-c-rpc-contract)、[`20260322103614_get_dashboard_overview_daily_by_domain.sql`](../../supabase/migrations/20260322103614_get_dashboard_overview_daily_by_domain.sql)）。適用は **Supabase MCP `apply_migration`**。手順・DoD は [US-C §4](./dashboard-overview-us-c.md#us-c-c1-migration)、[US-C §6・C1](./dashboard-overview-us-c.md#us-c-execution-plan)。
- [x] **C2 — 型**: [`supabase.types.ts`](../../src/features/links/types/supabase.types.ts) の RPC 戻り。詳細は [US-C §6・C2](./dashboard-overview-us-c.md#us-c-execution-plan)。
- [x] **C3 — API / Zod**: `daily_by_domain` 行の厳格スキーマ・テスト（[`fetchDashboardOverview.api.ts`](../../src/features/links/api/fetchDashboardOverview.api.ts)）。詳細は [US-C §6・C3](./dashboard-overview-us-c.md#us-c-execution-plan)。
- [x] **C4 — データ層**: `domainAddedStatsByDay` / `domainReadStatsByDay` / `domainStats` を `daily_by_domain` ピボットに；`useLinks({ limit: 500 })` およびドメイン経路のモック行列を撤去（[§6](./dashboard-overview-api.md)、[`useDashboardOverviewData.ts`](../../src/features/links/hooks/useDashboardOverviewData.ts)）。詳細は [US-C §6・C4](./dashboard-overview-us-c.md#us-c-execution-plan)。
- [x] **C5 — UI**: ドメイン内訳の `isTableLoading` に `dashboardOverviewPending` を合成（[`useDashboardBreakdownUi`](../../src/features/links/hooks/useDashboardBreakdownUi.ts)、[§6](./dashboard-overview-api.md)）。詳細は [US-C §6・C5](./dashboard-overview-us-c.md#us-c-execution-plan)。
- [x] **C6 — fixtures**: [`dashboardOverview.fixtures.ts`](../../src/features/links/testing/dashboardOverview.fixtures.ts)、[`dashboardOverview.fixtures.test.ts`](../../src/features/links/testing/__tests__/dashboardOverview.fixtures.test.ts)（`dashboardOverviewWithDomainBreakdownRpcFixture` の Zod 検証含む）。詳細は [US-C §6・C6](./dashboard-overview-us-c.md#us-c-execution-plan)。
- [x] **C7 — 品質ゲート**: `pnpm test` / `pnpm run check`。詳細は [US-C §6・C7](./dashboard-overview-us-c.md#us-c-execution-plan)。
...
| US-C | ドメイン母集団が `daily_totals` と同一で、抽出が `extractDomain` と同値                                              |
