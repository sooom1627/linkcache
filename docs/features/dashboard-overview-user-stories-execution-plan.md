...
**進捗サマリ**: **US-A（グラフ／`daily_totals`）は完了**（T1〜T9・2026-03-22 締め）。実装・検証の一次記録は [dashboard-overview-us-a.md](./dashboard-overview-us-a.md)（ストーリーステータス・§5〜§8）。**US-B** は **B1〜B7 完了**（[`dashboardOverview.fixtures.ts`](../../src/features/links/testing/dashboardOverview.fixtures.ts)・品質ゲート含む）。詳細は [dashboard-overview-us-b.md](./dashboard-overview-us-b.md) §5・§6。**US-C** は **C1（Supabase `daily_by_domain` RPC）完了**；**C2〜C7**（型・API・フック・fixtures 等）は未。一次記録は [dashboard-overview-us-c.md](./dashboard-overview-us-c.md)。
...
| **US-C** | domain_table（ドメイン別内訳）**— C1（RPC）済**、C2〜C7 未 | 同セクションで `tableView === "domain"`、`daily_by_domain`（C1 で RPC 実装済み）、クライアント接続は C4 以降。実行タスク一覧: [dashboard-overview-us-c.md](./dashboard-overview-us-c.md)                                                                     | §2, §3.2, §6 · [us-c.md](./dashboard-overview-us-c.md)    |
