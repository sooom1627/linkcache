...
**進捗サマリ**: **US-A（グラフ／`daily_totals`）は完了**（T1〜T9・2026-03-22 締め）。実装・検証の一次記録は [dashboard-overview-us-a.md](./dashboard-overview-us-a.md)（ストーリーステータス・§5〜§8）。**US-B** は **B1〜B7 完了**（[`dashboardOverview.fixtures.ts`](../../src/features/links/testing/dashboardOverview.fixtures.ts)・品質ゲート含む）。詳細は [dashboard-overview-us-b.md](./dashboard-overview-us-b.md) §5・§6。**US-C**（`daily_by_domain`・domain_table）の**コード実装は未**；垂直分割タスク **C1〜C7**・RPC 契約・DoD の一次記録は [dashboard-overview-us-c.md](./dashboard-overview-us-c.md)。
...
| **US-C** | domain_table（ドメイン別内訳）**— 実装未**（計画ドキュメントあり） | 同セクションで `tableView === "domain"`、`daily_by_domain`、[`extractDomain`](../../src/features/links/utils/urlUtils.ts) と SQL 同値、母集団は `new` / `done` のみ（§2）。実行タスク一覧: [dashboard-overview-us-c.md](./dashboard-overview-us-c.md)        | §2, §3.2, §6 · [us-c.md](./dashboard-overview-us-c.md)    |
...
- [ ] **C1 — Supabase**: `daily_by_domain` を RPC で返す；母集団 `new` / `done`、[`extractDomain`](../../src/features/links/utils/urlUtils.ts) と SQL 同値、上位 N ＋「その他」（[US-C §3](./dashboard-overview-us-c.md#us-c-rpc-contract)、[§2](./dashboard-overview-api.md)、[§3.2](./dashboard-overview-api.md)、[§3.3](./dashboard-overview-api.md#33-性能ダッシュボードモバイルで追加で押さえる点)）。適用は **Supabase MCP `apply_migration`** を正とする。マイグレーション手順は [US-C §4](./dashboard-overview-us-c.md#us-c-c1-migration)、タスク本文は [US-C §6・C1](./dashboard-overview-us-c.md#us-c-execution-plan)。
- [ ] **C2 — 型**: [`supabase.types.ts`](../../src/features/links/types/supabase.types.ts) の RPC 戻り。詳細は [US-C §6・C2](./dashboard-overview-us-c.md#us-c-execution-plan)。
- [ ] **C3 — API / Zod**: `daily_by_domain` 行の厳格スキーマ・テスト（[`fetchDashboardOverview.api.ts`](../../src/features/links/api/fetchDashboardOverview.api.ts)）。詳細は [US-C §6・C3](./dashboard-overview-us-c.md#us-c-execution-plan)。
- [ ] **C4 — データ層**: `domainAddedStatsByDay` / `domainReadStatsByDay` / `domainStats` を `daily_by_domain` ピボットに；`useLinks({ limit: 500 })` およびドメイン経路のモック行列を撤去；`buildDomainStatsFromLinks` はダッシュからの参照廃止（他未使用なら削除）（[§6](./dashboard-overview-api.md)、[`useDashboardOverviewData.ts`](../../src/features/links/hooks/useDashboardOverviewData.ts)）。詳細は [US-C §6・C4](./dashboard-overview-us-c.md#us-c-execution-plan)。
- [ ] **C5 — UI**: ドメイン内訳の `isTableLoading` に `dashboardOverviewPending` を合成（[`useDashboardBreakdownUi`](../../src/features/links/hooks/useDashboardBreakdownUi.ts)、[§6](./dashboard-overview-api.md)）。詳細は [US-C §6・C5](./dashboard-overview-us-c.md#us-c-execution-plan)。
- [ ] **C6 — fixtures**: [`dashboardOverview.fixtures.ts`](../../src/features/links/testing/dashboardOverview.fixtures.ts)、[`dashboardOverview.fixtures.test.ts`](../../src/features/links/testing/__tests__/dashboardOverview.fixtures.test.ts)。詳細は [US-C §6・C6](./dashboard-overview-us-c.md#us-c-execution-plan)。
- [ ] **C7 — 品質ゲート**: `pnpm test` / `pnpm run check`。詳細は [US-C §6・C7](./dashboard-overview-us-c.md#us-c-execution-plan)。
