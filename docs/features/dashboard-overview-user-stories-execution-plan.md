...
**進捗サマリ**: **US-A（グラフ／`daily_totals`）は完了**（T1〜T9・2026-03-22 締め）。実装・検証の一次記録は [dashboard-overview-us-a.md](./dashboard-overview-us-a.md)（ストーリーステータス・§5〜§8）。**US-B** は **B1〜B7 完了**（[`dashboardOverview.fixtures.ts`](../../src/features/links/testing/dashboardOverview.fixtures.ts)・品質ゲート含む）。詳細は [dashboard-overview-us-b.md](./dashboard-overview-us-b.md) §5・§6。**US-C** は **C1（RPC）・C2（型）・C3（API / Zod）完了**；**C4〜C7**（フック・fixtures・品質ゲートの残り）は未。一次記録は [dashboard-overview-us-c.md](./dashboard-overview-us-c.md)。
...
| **US-C** | domain_table（ドメイン別内訳）**— C1〜C3 済**、C4〜C7 未   | 同セクションで `tableView === "domain"`、`daily_by_domain`（C1 RPC・C2 型・C3 Zod 済み）、画面データ合成は C4 以降。実行タスク一覧: [dashboard-overview-us-c.md](./dashboard-overview-us-c.md)                                                               | §2, §3.2, §6 · [us-c.md](./dashboard-overview-us-c.md)    |
...
- [x] **C1 — Supabase**: `daily_by_domain` を RPC で返す；母集団 `new` / `done`、`extract_domain_for_dashboard` と [`extractDomain`](../../src/features/links/utils/urlUtils.ts) 同値、**N=15**・`__other__`（[US-C §3](./dashboard-overview-us-c.md#us-c-rpc-contract)、[`20260322103614_get_dashboard_overview_daily_by_domain.sql`](../../supabase/migrations/20260322103614_get_dashboard_overview_daily_by_domain.sql)）。適用は **Supabase MCP `apply_migration`**。手順・DoD は [US-C §4](./dashboard-overview-us-c.md#us-c-c1-migration)、[US-C §6・C1](./dashboard-overview-us-c.md#us-c-execution-plan)。
